...
describe('AppNix Installer', () => {
    ...
    it('should prepare directories and install AppImages, desktop entries, and icons', () => {
        ...
        expect(fs.chmodSync).toHaveBeenCalledWith(expect.stringContaining('/mock/.local/appnix/test-app/test-app.AppImage'), 0o755);
    ...
    it('should skip copying icon into /opt if specific icon does not exist', () => {
        ...
        expect(fs.copyFileSync).toHaveBeenCalledWith(expect.stringContaining('/workspace/dev/odr/cli/appnix/dist/test-app.desktop'), expect.stringContaining('/mock/desktop/test-app.desktop'));
        expect(fs.copyFileSync).not.toHaveBeenCalledWith(expect.stringContaining('/.config/appnix/icons/test-app.png'), expect.stringContaining('/mock/.local/appnix/test-app/test-app.png'));
    });
