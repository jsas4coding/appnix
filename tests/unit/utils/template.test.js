...
describe('Template Utility', () => {
    ...
    it('should generate Electron app files and write them to correct locations', async () => {
        ...
        expect(mockWriteFile).toHaveBeenCalledWith(path.join(getAppsPath(), mockApp.app_name, 'main.js'), expect.any(String), expect.any(Object));
        expect(mockWriteFile).toHaveBeenCalledWith(path.join(getAppsPath(), mockApp.app_name, 'package.json'), expect.any(String), expect.any(Object));
        expect(mockWriteFile).toHaveBeenCalledWith(path.join(getDesktopEntriesPath(), `${mockApp.app_name}.desktop`), expect.any(String), expect.any(Object));
        expect(mockConsoleLog).toHaveBeenCalledWith(expect.stringContaining('Generated Electron app files for:'), mockApp.name);
    });
    ...
    it('should handle template rendering errors', async () => {
        vi.spyOn(fs, 'readFile').mockRejectedValue(new Error('Read error'));
        await expect(renderTemplate('/mock/template/path', {})).rejects.toThrow('Failed to render template');
    });
});
