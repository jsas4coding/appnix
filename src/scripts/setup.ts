 ...
 * and creating a symlink at ~/.config/appnix/bin/appnix.
 */
export async function setupCli(): Promise<void> {
  ...
  // 6. Create symlink bin/appnix → ../lib/cli.js
  const symlinkPath = path.join(binPath, 'appnix');
  const symlinkTarget = path.join('..', 'lib', 'cli.js');
  ...
  console.log(`Make sure ${binPath} is in your PATH.`);
}
