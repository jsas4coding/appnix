...
|------|------|-------------|
| `src/types/app.ts` | Primary type definitions | `AppConfig`, `GlobalConfig` |
...
> **Note:** `src/types/config.ts` is the interface actively used at runtime via `@/types/config.js`. `src/types/app.ts` defines a more modular decomposition (`AppConfig`, `GlobalConfig`).
...
// Window hardcoded to 1280x800, menu bar always hidden
```
...
| `{{app_name}}` | `app.app_name` | Used in user agent cleanup regex |
...
// - Native context menu via webContents 'context-menu' event
...
// - User agent stripped of Electron/ and app name tokens
```
